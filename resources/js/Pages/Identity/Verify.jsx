  ...
  return (
    <GuestLayout>
      ...
      <p className="mt-2 text-center text-sm text-neutral-400">
        You can complete identity verification now, or skip and finish later
      ...
      <form onSubmit={submit} className="mt-8 space-y-4">
        <div className="grid grid-cols-2 gap-4">
          <div>
            ...
            <input
              ...
              onChange={(e) => setData('legal_first_name', e.target.value)}
            />
            <InputError message={errors.legal_first_name} className="mt-2" />
          </div>
          ...
          <div>
            ...
            <input
              ...
              onChange={(e) => setData('legal_last_name', e.target.value)}
            />
            <InputError message={errors.legal_last_name} className="mt-2" />
          </div>
        </div>
        ...
        <div className="grid grid-cols-2 gap-4">
          <div>
            ...
            <input value={email} disabled className={readOnlyFieldClasses} />
          </div>

          <div>
            <label className={labelClasses}>Mobile Number (Optional)</label>
            <input
              value={phone ?? ''}
              disabled
        ...
        <p className="text-xs text-neutral-500">
          Date of birth and address will be collected during intake.
        </p>
        ...
        {data.document_type && (
          <>
            <div className="grid grid-cols-2 gap-4">
              <div>
                <label className={labelClasses}>Document No</label>
                <input
              ...
              <div>
                ...
                <input
                  ...
                  className={fieldClasses}
                  onChange={(e) =>
                    setData('document_expiry_date', e.target.value)
                  }
                />
                <InputError
            ...
            <div className="grid grid-cols-2 gap-4">
              <PhotoUpload
                ...
                onChange={(file) => setData('front_photo', file)}
                error={errors.front_photo}
              />
              <PhotoUpload
                label="Document Back Photo"
                file={data.back_photo}
                onChange={(file) => setData('back_photo', file)}
...
function ProcessingView() {
  ...
  const percent = Math.round(
    (Math.min(stepIndex, PROCESSING_STEPS.length) / PROCESSING_STEPS.length) *
      100,
  );
    ...
    <GuestLayout>
      ...
      <div className="mt-8 rounded-md border border-neutral-800 bg-neutral-800/50 p-5">
        ...
        <p className="mt-1 text-xs text-neutral-500">
          This usually takes about 5-10 seconds. Please keep this page open.
        </p>
        ...
        <ul className="mt-4 space-y-2">
          {PROCESSING_STEPS.map((step, index) => (
            <li
              ...
              className={
                'text-sm ' +
                (index < stepIndex ? 'text-emerald-400' : 'text-neutral-500')
              }
            >
              {step}
            </li>
...
function RejectedView({ verification, onRetry }) {
  return (
    <GuestLayout>
      ...
      <p className="mt-2 text-center text-sm text-neutral-400">
        We couldn&apos;t verify your document. You can try again with a clearer
        photo.
      </p>
...
function VerifiedView({ verification, nextRoute }) {
  return (
    <GuestLayout>
      ...
      <p className="mt-2 text-center text-sm text-neutral-400">
        Your account and identity verification are complete. You&apos;re signed
        in and we&apos;ll take you back to where you started.
      </p>
      ...
      <div className="mt-8 rounded-md border border-neutral-800 bg-neutral-800/50 p-5">
        ...
        <dl className="mt-4 space-y-2 text-sm">
          <div className="flex justify-between border-b border-neutral-800 pb-2">
            ...
            <dd className="text-white">{verification.documentTypeLabel}</dd>
          </div>
          <div className="flex justify-between border-b border-neutral-800 pb-2">
            <dt className="text-neutral-500">Legal Name</dt>
            <dd className="text-white">{verification.legalName}</dd>
          </div>
          <div className="flex justify-between">
            <dt className="text-neutral-500">Verification Status</dt>
            <dd className="font-semibold text-emerald-400">Verified</dd>
          </div>
        </dl>
      </div>
